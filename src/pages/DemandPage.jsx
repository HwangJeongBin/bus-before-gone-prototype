...
const AGES = ["10대", "20대", "30대", "40대", "50대", "60대", "70대", "80대 이상"];

// 컴포넌트를 함수 밖으로 분리 — 리렌더링 시 포커스 유지
const Card = ({ children, style }) => (
  <div style={{ background: "#1E293B", borderRadius: 12, border: "1px solid #334155", padding: 20, ...style }}>
    {children}
  </div>
);
  ...
  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
    {options.map(o => (
      <button key={o} onClick={() => onChange(o)} style={{
        ...
        border: `1.5px solid ${value === o ? "#3B82F6" : "#334155"}`,
        background: value === o ? "#172554" : "transparent",
...
export default function DemandPage() {
  ...
  const [form, setForm] = useState({
    region: "", purpose: "", destination: "",
    frequency: "", access: "", phone: "", age: "", comment: ""
  });
  ...
  const handleSubmit = () => {
    ...
    setRecentList(prev => [
      { name: form.region, purpose: form.purpose, time: "방금 전", age: form.age || "미입력" },
      ...prev
    ]);
    setSubmitted(true);
  };
  ...
  const summaryItems = [
    ...
    { label: "이동 목적", value: form.purpose },
    { label: "주요 목적지", value: form.destination || "미입력" },
    { label: "이용 빈도", value: form.frequency || "미입력" },
    { label: "선호 접수", value: form.access || "미입력" },
    { label: "연령대", value: form.age || "미입력" },
  ];
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", padding: 40 }}>
        <div style={{ textAlign: "center", maxWidth: 400 }}>
          ...
          <div style={{ background: "#1E293B", borderRadius: 12, border: "1px solid #334155", padding: 16, marginBottom: 24, textAlign: "left" }}>
            {summaryItems.filter(i => i.value && i.value !== "미입력").map(i => (
          ...
          <button onClick={() => {
            setSubmitted(false);
            setStep(1);
            setForm({ region: "", purpose: "", destination: "", frequency: "", access: "", phone: "", age: "", comment: "" });
          }} style={{ padding: "12px 32px", borderRadius: 10, background: "#1D4ED8", border: "none", color: "white", fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
            추가 등록하기
          </button>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "24px 32px" }}>
          ...
          {[1, 2, 3].map((s, i) => (
            <div key={s} style={{ display: "flex", alignItems: "center" }}>
              <div style={{
                ...
                background: step >= s ? "#1D4ED8" : "#1E293B",
                border: `2px solid ${step >= s ? "#3B82F6" : "#334155"}`,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 13, fontWeight: 700, color: step >= s ? "white" : "#475569"
              }}>{s}</div>
              {i < 2 && <div style={{ width: 60, height: 2, background: step > s ? "#3B82F6" : "#334155" }} />}
            </div>
          ))}
          <div style={{ marginLeft: 12, fontSize: 13, color: "#64748B" }}>
            {["지역 및 목적", "이용 정보", "확인 및 제출"][step - 1]}
          </div>
        </div>
          ...
          <div>
            ...
            <Card style={{ marginBottom: 16 }}>
              ...
              <select
                ...
                style={{ ...inputStyle, color: form.region ? "#F1F5F9" : "#64748B" }}
              >
                ...
                return (
                  <div style={{ marginTop: 12, padding: "10px 12px", background: "#172554", borderRadius: 8, border: "1px solid #1D4ED844" }}>
                    <div style={{ fontSize: 12, color: "#93C5FD" }}>
                      AI 예측 위험도: <strong style={{ color: rc.color }}>{r.risk}% ({rc.label})</strong>
                      {" "}| DRT 수요 등록: <strong style={{ color: "#60A5FA" }}>{r.drtDemand}명</strong>
                    </div>
                  </div>
                );
              })()}
            ...
            <Card style={{ marginBottom: 24 }}>
              ...
              <input
                ...
                placeholder="예: 의성군 보건의료원, 의성 홈플러스"
                style={inputStyle}
            ...
            <button
              onClick={() => form.region && form.purpose && setStep(2)}
              style={{
                padding: "12px 32px", borderRadius: 10, border: "none", fontSize: 14, fontWeight: 600,
                background: form.region && form.purpose ? "#1D4ED8" : "#1E293B",
                color: form.region && form.purpose ? "white" : "#475569",
                cursor: form.region && form.purpose ? "pointer" : "default"
              }}
            >
              다음 단계 →
            </button>
        ...
        {step === 2 && (
          <div>
            ...
            <Card style={{ marginBottom: 16 }}>
              <Label>연령대 (선택)</Label>
              <ChipGroup options={AGES} value={form.age} onChange={v => update("age", v)} />
            </Card>
            ...
            <Card style={{ marginBottom: 24 }}>
              ...
              <textarea
                ...
                placeholder="버스 이용에 불편했던 점이나 DRT에 바라는 점을 자유롭게 적어주세요"
          ...
          <div>
            ...
            <Card style={{ marginBottom: 24 }}>
              ...
              <div style={{ fontSize: 11, color: "#475569", marginTop: 6 }}>
                ※ 개인정보는 DRT 도입 현황 안내 목적으로만 활용되며 제3자에게 제공되지 않습니다.
              </div>
            </Card>
